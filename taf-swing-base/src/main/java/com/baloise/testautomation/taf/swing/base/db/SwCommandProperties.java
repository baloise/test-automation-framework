...
public class SwCommandProperties extends H2Table {
  ...
  static void createTable() {
    String sql = "CREATE TABLE  IF NOT EXISTS COMMAND_PROPERTIES (c_id int, c_key varchar(255), c_value varchar(2000));";
    ...
    try {
      ...
      error("error creating table 'command_properties'", e);
    }
    finally {
  ...
  public static TafProperties getTafPropertiesForId(int id) {
    ...
    PreparedStatement ps = null;
    try {
      ps = conn().prepareStatement("select c_key, c_value from command_properties where c_id = ?");
      ps.setInt(1, id);
      rs = ps.executeQuery();
      if (rs != null) {
        while (rs.next()) {
  ...
  public static void listProperties() {
    ...
    PreparedStatement ps = null;
    try {
      ps = conn().prepareStatement("select c_key, c_value, c_id from command_properties");
      rs = ps.executeQuery();
      if (rs != null) {
        while (rs.next()) {
  ...
  private void insert() {
    String insertSQL = "INSERT INTO COMMAND_PROPERTIES (c_id, c_key, c_value) VALUES (?, ?, ?)";
    PreparedStatement ps = null;
    try {
